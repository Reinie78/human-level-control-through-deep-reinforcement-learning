...
class PCNNLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(self.iterations):
            # Linking field: convolution over the firing map
            if t > 0:
            ...
            # Update θ: decay + jump where y fired
            theta = self.threshold_decay * theta + y

            # Optional: Accumulate output firing map
            y_out += y

        return y_out / self.iterations  # Normalize over time
